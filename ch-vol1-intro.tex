...
Mark Meilstrup, Harold P.\ Boas, and Atilla Y{\i}lmaz
...
By ``$:=$'',\glsadd{not:def} we mean we are defining what $S$ is, rather than
...
belong to $S$.  We write $T \subset S$.\glsadd{not:subset}  More formally we make the
...
$f \colon A \to B$.\glsadd{not:func}  For example, we
...
the \emph{\myindex{image}} (or \emph{\myindex{direct image}}) of $C$ as
\glsadd{not:dirimage}
\begin{equation*}
f(C) := \{ f(x) \in B : x \in C \} .
\end{equation*}
...
numbers, or $\{ a,c \} \subset \{ a,b,c \}$ for sets.  These are examples of
relations.
...
First, let the relation be `$<$'.  Then the corresponding set 
of pairs is $\{ (1,2), (1,3), (2,3) \}$.  So $1 < 2$ holds as $(1,2)$ is in
the set, but $3 < 1$ does not hold as $(3,1)$ is not in the set.

Similarly, the relation `$=$'
is defined by the pairs $\{ (1,1), (2,2), (3,3) \}$.

Any subset of $A \times A$ is a relation.  Let us define the relation
$\dagger$ via $\{ (1,2), (2,1), (2,3), (3,1) \}$, then $1 \dagger 2$ and
$3 \dagger 1$ are
...
Finally, a relation `$\star$' defined by $\{ (1,1), (1,2), (2,1), (2,2), (3,3) \}$ is
...
In particuarl note that $\abs{A} = 2$ and $\abs{\sP(A)} = 4 = 2^2$.
In fact, in general,
...
in \exerciseref{example:ratnums} is an equivalence
relation.
\end{exercise}
