...
%mbxlatex       the author was in part supported by NSF grants DMS-0900885 and
...
\emph{complex analysis}, really builds upon the present material, rather than
...
We talk about sets in a rather informal way, using what is usually called
...
\begin{myfigureht}
%mbxalt A diagram of example sets S and T.  The set S includes 0, 1, and 2,
%mbxalt and the set T includes 0 and 2.  There is an element 7 not
%mbxalt included in either set.
...
\begin{myfigureht}
%mbxalt Venn diagrams of set operations: First a diagram of A union B,
%mbxalt with the inside of A or B is shaded.
%mbxalt Then a diagram of A intersect B where only the intersection
%mbxalt A and B is shaded.  Then a diagram of A setminus B, where
%mbxalt everything in A but not in B is shaded.  Finally a diagram
%mbxalt of B complement, where everything not in B is shaded.
...
\begin{myfigureht}
%mbxalt Two sets of dots are labeled 1, 2, 3, 4 and a, b, c, d.
%mbxalt There is an arrow from 1 to b, from 2 to d, from 3 to c,
%mbxalt and from 4 to b.  A list of example images is on the right:
%mbxalt f({1,2,3,4}) = {b,c,d},
%mbxalt f({1,2,4}) = {b,d},
%mbxalt f({1}) = {b},
%mbxalt f^{-1}({a,b,c}) = {1,3,4},
%mbxalt f^{-1}({a}) = empty set,
%mbxalt f^{-1}({b}) = {1,4}.
...
\begin{myfigureht}
%mbxalt A grid of pairs (row,column) is given with an arrow
%mbxalt going from (1,1) to (1,2), then to
%mbxalt (2,1) diagonally down and left, then to (1,3), then diagonally
%mbxalt down and left to (2,2) and then to (3,1), then
%mbxalt an arrow to (1,4), then diagonally down and left to (2,3),
%mbxalt then (3,2), then (4,1).  Dots indicate the diagram is infinite.
