...
Ru Wang, Trevor Fancher, Brandon Tague,
...
For $S$ and $T$ defined above $T \subset S$, but
...
S$ by assumption.  So $m > 1$ and $m-1$ is a natural number as well.
...
For instance, the set $[0,1] \times [0,1]$ is a set in the plane
...
\begin{defn}
Consider a function $f \colon A \to B$ and $C \subset A$.  Define
...
Notice in particular that $R(f) = f(A)$, the range is the direct image of
the domain $A$.
...
f^{-1}( C \cup D)$, meaning that
$x$ is taken to $C$ or $D$.  Thus
...
If $f$ is both an surjective and injective, then
