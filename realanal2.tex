...
material and many of the proofs are very similar as in Rudin's book.
...
used a few times in examples, although they are heavily used in
\chapterref{approx:chapter}.  It is entirely possible to avoid using all
...
\begin{enumerate}[1)]
\item \ref{sec:vectorspaces}--\ref{sec:svinvfuncthm},
(perhaps \ref{sec:diffunderint}), \ref{sec:rirect} and \ref{sec:iteratedints}.
\item \ref{sec:vectorspaces}--\ref{sec:mvhighordders},
\ref{sec:diffunderint}--\ref{sec:pathind}, \ref{sec:rirect} and \ref{sec:iteratedints}.
\item Everything in chapters \ref{pd:chapter}, \ref{path:chapter}, and
\ref{mi:chapter}.
\item Chapters \ref{pd:chapter}, (maybe \ref{path:chapter}), and
\ref{approx:chapter}.
\end{enumerate}
