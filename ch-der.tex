...
\propref{prop:compositioncont}, and the limit is
$Q\bigl(g(y)\bigr) = Q(x) = \nicefrac{1}{f'(x)}$.
In other words,
%as
%$g(s) \to g(y)$ as $s \to y$,
...
increasing (this can also be proved directly), so $f$ is one-to-one.
Given any $M > 1$, $f(M) = M^n \geq M$, and given any $1 > \epsilon > 0$,
$f(\epsilon) = \epsilon^n \leq \epsilon$.  For any $y$ with $\epsilon < y <
M$ we have by the
\hyperref[IVT:thm]{intermediate value theorem} we find $y \in f\bigl(
(0,\infty) \bigr)$.  As $M$ and $\epsilon$ were arbitrary, $f$ is onto
