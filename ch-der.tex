...
the \emph{\myindex{secant line}}.  When we take the limit as $x$ goes to $c$,
...
Two diagrams of a plot of a function f.  One the left,
the points c and x are marked on the horizontal axis.
The corresponding points on the graph of f are marked
and through them is drawn a line (a secant line).  It is labeled
with slope equals f of x -f of c the entire thing divided
by quantity x minus c.
On the right, the same function is drawn but only the point c
is marked.  A line that is tangent to the graph of f
through the corresponding point is drawn and labeled
with slope equals f prime of c.}
...
\begin{example}
Let $f(x) \coloneqq x^2$ be defined on the whole real line.  Let $c \in \R$ be arbitrary.  We find that if
$x \neq c$,
\begin{equation*}
\frac{x^2-c^2}{x-c} =
\frac{(x+c)(x-c)}{x-c} =
...
\begin{example}
The function $f(x) \coloneqq \sabs{x}$ is not differentiable
...
\begin{prop}
If $f \colon I \to \R$ is differentiable at $c \in I$,
...
Instead, we get what is called the \emph{product rule}
or the \emph{\myindex{Leibniz rule}}%
...
A diagram of a rectangular area in the plane, divided
into 3 rectangles.  The horizontal side runs from 0 to
f of x, and the vertical side runs from 0 to g of x.
The largest white area is a rectangle that runs horizontally
from 0 to some f of c, which is slightly less than f of x,
and vertically from 0 to some g of c, which is slightly less than
g of x.  This white area is labeled as f of c times g of c.
On top, there is a thin lightly shaded rectangle running
on the horizontal side from 0 to f of x, and on the vertical
side from g of c to g of x.  It is labeled 
as f of x times the quantity g of x minus g of c.
Finally, there is a thin darkly shaded rectangle
running horizontally from f of c to f of x and vertically
from 0 to g of c.  It is labeled as f of x minus f of c
the entire thing times g of c.}
...
$\bigl( f(x) - f(c) \bigr) g(c)$.
In other words, roughly,  $\Delta (f \cdot g)
...
\begin{exercise}
Assume the inequality $\babs{x-\sin(x)} \leq x^2$.  Prove that $\sin$ is
...
$n < 0$, assume $f(x) \neq 0$ for all $x \in I$.  Prove that
...
$f'(c) \neq 0$ and $g$ is differentiable at $f(c)$.  Use the chain rule
...
\begin{enumerate}[a)]
\item
Suppose $f \colon (-1,1) \to \R$ is a function such that $f(x) = x h(x)$ for
a bounded function $h \colon (-1,1) \to \R$.
...
$\babs{a+b(x-c) - f(x)} \leq \epsilon \sabs{x-c}$, whenever $x \in I$ and
$\sabs{x-c} < \delta$.
...
is it gets close to the origin, looking somewhat like the graph
...
relatively close between two two points where the sine
...
A dashed line singnifying the degree 20 approximation is drawn and
...
at the origin, the cube root is a reflection accross the x equals y
