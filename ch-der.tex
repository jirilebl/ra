...
Let us also give a quick application of Taylor's theorem you remember from
calculus.  By a \emph{\myindex{strict relative minimum}} of $f$ at $c$,
...
$c \in (a,b)$, and $f \colon (a,b) \to \R$ is $n$ times continuously
differentiable, with $f^{(k)}(c) = 0$ for $k=1,2,\ldots,n-1$ and $f^{(n)}(c)
\not= 0$.
Prove\\
a)~If $n$ is odd then $f$ does has neither a relative minimum,
nor a maximum at $c$.
\\
b)~If $n$ is even then $f$ has a strict relative minimum at $c$ if $f^{(n)}(c) > 0$
and a strict relative maximum at $c$ if $f^{(n)}(c) < 0$.
\end{exercise}
...
Let us start with a simple example.  Suppose a function $f(x) = a x$ for a
number $a \not= 0$.  Then $f \colon \R \to \R$ is bijective, and the inverse
is $f^{-1}(y) = \frac{1}{a} x$.  In particular, $f'(x) = a$ and 
...
Let $x,y$ be as in the statement, take $t \in I$, $t \not= x$, to be arbitrary
...
As a function of $t$, the expression
$\frac{t-x}{f(t)-f(x)}$ is continuous at $x$.
As $g$ is continuous at $y$, and $g(y) = x$,
the composition is continuous at $y$ by
\propref{prop:compositioncont}.  That is, as
$g(s) \to g(y)$ as $s \to y$, we 
...
If $f'$ are continuous and $f'$ is nonzero at all $x$,
