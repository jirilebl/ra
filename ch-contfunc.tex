...
it exists, is unique, which needs to be proved.
Let us do that now.
...
\myquote{$\{ f(x_n) \}_{n=1}^\infty$ converges,} without requiring a specific limit.
...
We say
$f$ is \emph{continuous at $c$}\index{continuous at $c$}
...
Proof:
If $c$ is rational, take a sequence $\{ x_n \}$
...
If $y < 1$, then notice that $f(1) = 1^k -y > 0$.  If $y > 1$,
then notice that $f(y) = y^k-y = y(y^{k-1}-1) > 0$.
Apply Bolzano's theorem to find an $x$ such that $f(x) = 0$,
