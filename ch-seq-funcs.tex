...
simplest is perhaps to combine $\sabs{x}$ and $\frac{n}{2}x^2 +
...
\end{equation*}
where $\snorm{f_n-f_k}_{[-h,h]}$ is the uniform norm, that
is the supremum of $\babs{f_n(t)-f_k(t)}$ for $t \in [-h,h]$.
...
L \snorm{f_n-f}_{[-h,h]} .
\end{equation*}
As $\snorm{f_n-f}_{[-h,h]}$ converges to 0, then
...
so $f$ is only a what is called a \emph{\myindex{weak solution}} to the
