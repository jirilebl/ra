...
See the exercises for more counterexamples.  Using 
...
Let us show that it also converges uniformly.  This fact allows us to
...
at all.
\item
Show that the joint limit of $\{ x_{n,m} \}$ exists and is 0.
\end{enumerate}
\end{exercise}
...
\item
Using part a) prove a version of the so called ``identity
...
continuous.  Furthermore since all the $f_n([-h,h]) \subset
...
Therefore to obtain existence of a solution, some sort of
continuity hypothesis on
$F$ is necessary.
\end{example}

\begin{remark}
It is possible to weaken what we mean by ``solution to $y' = F(x,y)$''
...
\href{http://en.wikipedia.org/wiki/Oliver_Heaviside}{Oliver Heaviside}
(1850--1825).},
...
To solve the equation from the previous example in this way, 
we would require a more powerful integral, such as the Lebesgue integral.
\end{remark}
