...
Limits of sequences of numbers are unique, and so if a sequence $\{ f_n \}$
...
$\{ f_n \}$ converges uniformly to 0 on $[-a,a]$.  Note that
...
instead of \emph{converges uniformly} if $\snorm{f_n-f} \to 0$.  The proposition
...
\end{split}
\end{equation*}
Therefore, $\bigl\{ f(x_m) \bigr\}$ converges to $f(x)$ and $f$ is continuous at
...
Therefore, the limit as $n \to \infty$ on the right-hand side exists.
Define $f$ at the remaining points by this limit:
\begin{equation*}
f(x) :=
