...
a)~Show that for the initial condition $y(1)=1$, Picard's theorem applies.
Find an $\alpha > 0$, $M$, $L$, and $h$ that would work in the proof.\\
b)~Show that for the initial condition $y(1) = 0$, Picard's theorem
does not apply.\\
c)~Find a solution for $y(1) = 0$ anyway.
\end{exercise}
