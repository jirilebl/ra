...
Graphs of the functions x squared, x to the four, x to the six, and
x to the sixteen.  The graphs are over the region from minus 1 to 1,
and all start at the same point on the left-hand side and all end
at the same point on the right-hand side (as the functions are all 1
at the endpoints).  Inside the interval the higher powers make the graph
flatter and closer to the x-axis in the middle, but they have to then rise more
steeply back to 1 when we get to the ends of the interval.}
...
When $x = 1$ or $x=-1$, we have $x^{2n} = 1$ for all $n$ and hence
$\lim_{n\to\infty}f_n(x) = 1$.

Note that for $x$ not in $[-1,1]$, the sequence
$\{ x^{2n} \}_{n=1}^\infty$ does not converge.
So if we defined $f_n$ on a set larger than $[-1,1]$,
then $\{ f_n \}_{n=1}^\infty$
would converge pointwise only on $[-1,1]$.
\end{example}
...
Graphs of a number of sine waves, with higher frequencies in lighter
gray.  It is almost like we are trying to fill out
a horizontal strip by drawing wavy lines.
The all the curves start at the same point on the
x-axis on the left-hand side
going upwards, and all go through same point on the x-axis on
the right hand side going up.
All the curves go through the center on the x-axis, but some upwards 
and some downwards.
Similar patterns repeat at other points, but only some
of the higher frequency lines go through those points.}
...
an $N \in \N$ such that
\begin{equation*}
\babs{f_n(x)-f(x)} < \epsilon \quad \text{for all } n \geq N .
\end{equation*}
\end{prop}
...
for all $n \geq N$,
\begin{equation*}
\babs{f_n(x) - f(x)} < \epsilon \qquad \text{for all } x \in S.
\end{equation*}
\end{defn}
\begin{myfigureht}
\myincludegraphics{uniformconv}{%
A graph of a function marked f with a strip marked around the function
with its boundary marked f minus epsilon and f plus epsilon.  A function
marked f sub n is shown as a wiggly line that fits entirely within
this strip.}
...
to exist an $N$ such that $x^{2N} = \sabs{x^{2N} - 0} < \nicefrac{1}{2}$ for all $x \in
...
Sometimes other notation\footnote{The notation and terminology
are not completely standardized.  The norm is also called the
...
The subscript is the set over which the supremum is taken.
So if $K \subset S$, then
\begin{equation*}
\snorm{f}_K =
...
\begin{proof}
First suppose 
$\lim_{n\to\infty} \snorm{f_n - f}_S = 0$.  Let $\epsilon > 0$ be
given.  There exists an $N$ such that
for $n \geq N$, we have $\snorm{f_n - f}_S < \epsilon$.  As $\snorm{f_n-f}_S$
is the supremum of $\babs{f_n(x)-f(x)}$, we see that for all $x \in S$,
we have $\babs{f_n(x)-f(x)} \leq \snorm{f_n - f}_S < \epsilon$.

On the other hand, suppose $\{ f_n \}_{n=1}^\infty$ converges uniformly to $f$.
Let $\epsilon > 0$ be given.  Then find $N$ such that 
for all $n \geq N$, we have
$\babs{f_n(x)-f(x)} < \epsilon$ for all $x \in S$.
...
Using the uniform norm, we define Cauchy sequences similarly to
how we define Cauchy sequences of real numbers.
...
as $k$ goes to infinity.  Then $\babs{f_m(x)-f_k(x)}$
goes to $\babs{f_m(x)-f(x)}$.
...
$\left\{ \dfrac{e^{x/n}}{n} \right\}_{n=1}^\infty$ for $x \in \R$.
\item
...
\begin{exercise}
Find an example of a pair of sequences of functions $\{ f_n \}_{n=1}^\infty$ and
$\{ g_n \}_{n=1}^\infty$
...
that is $\babs{f_n(x)} \leq B$ for all $x \in S$.
...
downward line from the point (0,n) down to the x-axis
...
a sine wave for 10 different frequencies, but now it is also scaled
down for higher frequencies, so now the higher frequencies are 
closer to the x-axis, so it no longer fills a strip.}
...
On the left is a plot of a bell-shape curve, with the peak being narrower
...
On the right half of the point they point upward, and
