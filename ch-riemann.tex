...
If $\babs{f(x)} \leq M$ for all $x \in [a,b]$, then
\begin{equation*}
...
A diagram of a graph of a function that is 1 from 0 to 1
at 1 it is one half, and from 1 to 2 the function is 0.
The horizontal axis is divided into three intervals,
from 0 to 1 minus epsilon, then to 1 plus epsilon, then to 2.
Their lengths are Delta x sub 1 equals 1 minus epsilon,
Delta x sub 2 equals 2 epsilon, and 
Delta x sub 3 equals 1 - epsilon, respectively.
There is a shaded rectangle of height 1 for x between 0 and
1 minus epsilon.  There is a white rectangle of height 1
for x between 1 minus epsilon to 1 plus epsilon.
On the vertical axis,
the lower point is marked as M sub 3 equals m sub 2
equals m sub 3 equals 0.
The upper point is marked as M sub 1 equals M sub 2
equals m sub 1 equals 1.}
...
$a \nless b$.  Suppose $b < a$ and $f \in \sR\bigl([b,a]\bigr)$.
Define
\begin{equation*}
...
for all $i$, then
$\babs{L(P,f) - I} < \epsilon$ and
$\babs{U(P,f) - I} < \epsilon$.  If $f \in \sR\bigl([a,b]\bigr)$, then $I = \int_a^b f$.
\end{exercise}
...
\end{equation*}
As $f(x) \leq g(x)$, we have $m_i \leq \widetilde{m}_i$.
Therefore,
\begin{equation*}
L(P,f)
=
...
\begin{proof}
Let $M > 0$ be a real number such that $\babs{f(x)} \leq M$.
As $(b-a) \geq (b_n-a_n)$,
\begin{equation*}
-M(b-a) \leq
-M(b_n-a_n) \leq
...
Prove, without using \propref{prop:upperlowerlinineq}, that $f+g$ is in
...
Show that as a function of $x$ the expression $\babs{f(x)}$ is bounded with finitely many
...
That is,
define $f \colon [0,1] \to \R$ by $f(0)=1$ and for $x > 0$ by
\begin{equation*}
f(x) \coloneqq 
\begin{cases}
...
\begin{exercise}
Using \exerciseref{exercise:stepfunctionintegrable}, show that a step
...
Suppose $f \in \sR\bigl([a,b]\bigr)$.  Prove that the function that takes $x$ to
$\babs{f(x)}$ is also Riemann integrable on $[a,b]$.
...
Prove a version of the \emph{\myindex{Riemann--Lebesgue Lemma}}
...
of the origin, and sort of like minus sin to the left of the
...
A graph of several functions on the intreval
...
the right-hand and point and the middle point pi times quantity 2 n
...
On the right half of the integral, a dotted line denotes
the same function as before, but now it is below the solid line.
