...
Then $\gamma$ is said to be
\emph{continuously differentiable}
whenever $\gamma_j$ is a continuously differentiable function
for every $j=1,2,\ldots,n$, that is
$\gamma_j^{\:\prime}(t) = \lim_{h \to 0} \frac{\gamma(t+h)-\gamma(t)}{h}$
exists for all $t \in [a,b]$, and is continuous.
This definition is a straightforward 
extension of the definition of the derivative on an open set.
As before we treat
$\gamma^{\:\prime}(t)$ either as an $n \times 1$ matrix (a linear operator) or
a vector, as it is a column vector.
