...
word ``smooth'' can sometimes mean
``infinitely differentiable'' in the literature.}
if
...
a vector, as it is an $n \times 1$ matrix---a column vector.
...
The definition of piecewise smooth path as we have given it above implies
continuity (exercise).  Note that some authors allow also finitely many
discontinuities.
One could mostly get by with smooth paths, but for computations, the simplest
...
not differentiable.
%The restrictions are differentiable and the
...
$\gamma\bigl([a,b]\bigr)$
has no ``corners'' where $\gamma$ is smooth.
Consider 
\begin{equation*}
\gamma(t) :=
\begin{cases}
...
If $t \in [s_{j-1},s_j]$, then $h(t) \in [t_{j-1},t_j]$, so
...
Similarly for piecewise smooth we get
...
1.  If $\gamma \colon [0,r] \to \R^n$ is arclength parametrization, it is
common then to use $s$ as the variable as then we have $\int_\gamma f ~ds
