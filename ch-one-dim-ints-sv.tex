...
\end{equation*}
Differentiation is a limit and therefore we are really asking when the
two limiting operations of integration and differentiation commute.
It is not always possible to commute them and some extra hypothesis is
necessary.  The first
...
Surface plot of a function in the first quadrant on the xy-plane.
The function is zero on both the x and the y axes, however it rises very
steeply from the y-axis.}
...
Surface plot of a function on the unit square.  On the axes the
function is zero and is otherwise positive, except at the origin,
where there seems to be a discontinuity and it appears 
that as we move from the inside of the square towards the origin we would
attain a positive value rather than zero.}
...
\end{equation*}
Therefore, $g'(y)$ exists and is the continuous function
\begin{equation*}
g'(y) =
...
In other words, there exists a continuous function $\gamma' \colon [a,b]
\to \R^n$ such that for every $t \in [a,b]$, we have
$\lim\limits_{h \to 0}
\frac{\snorm{\gamma(t+h)-\gamma(t) - \gamma'(t) \, h}}{\sabs{h}} = 0$.
We treat
$\gamma'(t)$ either as a linear operator (an $n \times 1$ matrix) or
a vector,
$\gamma'(t) =
...
the restriction $\gamma|_{[t_{j-1},t_j]}$ is a smooth path for every
$j=1,2,\ldots,k$.
...
A diagram of a square in the xy-plane with arrows showing
that at t equals 0 we are at the origin.
Then there is an arrow rightwards
to (1,0) where we are at t equals 1.
Then there is an arrow upwards
to (1,1) where we are at t equals 2.
Then there is an arrow leftwards
to (0,1) where we are at t equals 3.
Finally, there is an arrow downwards to the
origin where we are at t equals 4.}
...
$(\gamma|_{[1,2]})'(t) = (0,1) \neq 0$.  Similarly for the other three sides.
Notice
that
$(\gamma|_{[1,2]})'(1) = (0,1)$,
$(\gamma|_{[0,1]})'(1) = (1,0)$, but
$\gamma'(1)$ does not exist.  At the corners $\gamma$ is 
not differentiable.
...
\myquote{corner} at the origin.  And that is because $\gamma'(0) = (0,0)$.
More complicated examples with, say, infinitely many corners exist,
see the exercises.
\begin{myfigureht}
\myincludegraphics{cornersmoothpath}{%
A diagram of a path in the plane.  The path has an arrow indicating
the following movement.  Starting above the origin on the y-axis
a point where t equals minus 1 is marked.  The path moves along the y-axis
downwards and then, much closer to the
origin, is a point marked t equals minus one half.
The origin is labeled t equals 0.  The path suddenly changes direction
rightwards to go along the x-axis.  A point near the origin is
marked t equals one half, then some distance further is a point marked t
equals 1.}
\caption{\myquote{Smooth} path with a corner if we allow zero derivative.  The
points corresponding to several values of
$t$ are marked with dots.\label{fig:cornersmoothpath}}
\end{myfigureht}
\end{example}

The condition $\gamma'(t) \neq 0$ even at the endpoints of
every subinterval guarantees
not only no corners, but also that the path ends nicely, that is, it can
extend a little bit past the endpoints if needed.  Again, see the exercises.
...
\end{equation*}
Then $\gamma'(t) = \bigl( 1 , f'(t) \bigr)$, which is never zero,
...
and $\bigl(h|_{[t_{j-1},t_j]}\bigr)'(t) \neq 0$
for all $t \in [t_{j-1},t_j]$.
Since $h$ is bijective, it is either strictly increasing or
strictly decreasing.  So either $\bigl(h|_{[t_{j-1},t_j]}\bigr)'(t) > 0$
for all $t$ or $\bigl(h|_{[t_{j-1},t_j]}\bigr)'(t) < 0$ for all $t$.
...
The notation for an integral of a one-form makes sense from the
\myquote{$u$-substitution} formula you remember from calculus.
Somewhat informally,
if $x_j(t) = \gamma_j(t)$, then $dx_j = \gamma_j'(t) \, dt$.
...
A path going counterclockwise around the origin.
The point on the positive x-axis is marked as the start
and is labeled as
gamma of 0 equals alpha of 0 equals beta of 2 pi.
The next point on the path in the first quadrant
halfway to the positive y-axis is labeled as
gamma of pi over 4 equals alpha of one eighth equals beta of 7 pi over 4.
The next point on the positive y-axis is labeled as
gamma of pi over 2 equals alpha of one quarter equals beta of 3 pi over 2.
The next point on the negative x-axis is labeled as
gamma of pi equals alpha of one half equals beta of pi.
The next point on the negative y-axis is labeled as
gamma of 3 pi over 2 equals alpha of three quarters equals beta of pi over 2.
The last point, which is the same as the first point,
is labeled as
gamma of 2 pi equals alpha of one equals beta of 0.}
...
circle once in the counterclockwise direction, and $\beta$ goes around the
...
The point on the positive x-axis is marked as the start
and is labeled as
gamma of 0 equals beta of 0 equals beta of pi.
The next point on the path in the first quadrant
halfway to the positive y-axis is labeled as
gamma of pi over 4 equals beta of pi over 8 equals beta of 9 pi over 8.
The next point on the positive y-axis is labeled as
gamma of pi over 2 equals beta of pi over 4 equals beta of 5 pi over 4.
The next point on the negative x-axis is labeled as
gamma of pi equals beta of pi over 2 equals beta of 3 pi over 2.
The next point on the negative y-axis is labeled as
gamma of 3 pi over 2 equals beta of 3 pi over 4 equals beta of 7 pi over 4.
The last point, which is the same as the first point,
is labeled as
gamma of 2 pi equals beta of 2 pi.}
...
$\gamma'(t) = 0$ for all $t$, and $\gamma\bigl([a,b]\bigr)$ is a single
...
Next, we integrate a function against the
...
The integral is the area of the fence.
See \figureref{fig:fenceintegral}.
\begin{myfigureht}
\myincludegraphics{fenceintegral}{%
A diagram in the xyz-space.  A curve gamma is given on
the xy-plane in a bold line.  A fence-like (vertical) shaded surface is
raised above this curve, which has varying heights above different
points.}
...
$\gamma(t) \coloneqq \bigl(\cos(t),\sin(t)\bigr)$.
Then $\gamma'(t) = \bigl(-\sin(t),\cos(t)\bigr)$, and
\begin{equation*}
\int_C f \, ds =
\int_\gamma f \, ds
=
...
$\int_a^b
\bnorm{\gamma'(t)}\, dt$ even if the path is only piecewise smooth.
That is because $\bnorm{\gamma'(t)}$ is defined and continuous
...
Then $\gamma'(t) = y-x$, and therefore
\begin{equation*}
\ell\bigl([x,y]\bigr)
=
\int_{[x,y]} ds
...
A diagram of a straight line path from a point x
to the point y, with this direction is indicated by an arrow.
The path is labeled [x,y].  The point x is also labeled as t equals 0
and the point y is also labeled as t equals 1.}
...
that $\bnorm{\gamma'(t)} = 1$ for all $t$.
Similarly for piecewise smooth $\gamma$, we get
$\bnorm{\gamma'(t)} = 1$ for all $t$ where the derivative exists.
Think of such a parametrization as moving around your curve at speed
...
Show that there exists an $\epsilon > 0$ and a continuously differentiable function
...
for
$t \neq 0$ and $\gamma(0) = (0,0)$.  Show that
\begin{enumerate}[a)]
\item
...
\item
Show that there is no piecewise smooth path $\alpha$ whose image equals
...
Let $U \subset \R^n$ be an open set and $\omega$ a one-form defined on $U$.
...
A diagram where the horizontal direction is labeled as e sub j.
A point p is marked, and then a path going in a curved way to a
point marked x.  Then a straight line path rightwards (horizontally)
goes to the point x plus h times e sub j.}
\caption{Using path independence in computing the partial
derivative.\label{fig:pathindantider}}
\end{myfigureht}


Since $U$ is open, suppose $h$ is so small that all points of distance
$\sabs{h}$ or
...
$\babs{\omega_j(x)-\omega_j(y)} < \epsilon$ whenever $\snorm{x-y} \leq \sabs{h}$.
Thus,
$\babs{\omega_j(x+the_j)-\omega_j(x)} < \epsilon$ for all $t \in [0,1]$,
and we estimate
\begin{equation*}
...
A diagram of two points, x and y, and two different paths, alpha and beta,
both starting at x and ending at y.}
...
There is an easier local criterion, a differential equation, that guarantees
...
function $f$ locally.
We can find the antiderivative in every
\emph{\myindex{simply connected}} domain, which informally
is a connected open set where
...
the result for what are called star-shaped domains, which is often good enough.
...
A picture of a shaded set which vaguely looks like a star.  A point p is
marked as well as a number of other random points in the set (one of which
is labeled x).  Each of these points is connected by a line segment with p,
and we note that the entire line segment stays in the set.}
\caption{A star-shaped domain with respect to $p$.\label{fig:starshaped}}
\end{myfigureht}
\end{defn}

Notice the difference between star-shaped and convex.  Convex implies
...
differentiable one-form defined on $U$.  That is,
\begin{equation*}
\omega =
\omega_1 \,dx_1 +
\omega_2 \,dx_2 + \cdots +
\omega_n \,dx_n ,
\end{equation*}
where $\omega_1,\omega_2,\ldots,\omega_n$ are continuously differentiable
...
$\gamma(t) \coloneqq (1-t)p + tx$, so $\gamma'(t) = x-p$.  Let
\begin{equation*}
...
\begin{example}
Without some hypothesis on $U$, the theorem is not true.
On $\R^2 \setminus \{ (0,0) \}$, let
\begin{equation*}
\omega(x,y) \coloneqq \frac{-y}{x^2+y^2} \,dx + \frac{x}{x^2+y^2} \,dy
\end{equation*}
Then
...
\end{equation*}
However, there is no $f \colon \R^2 \setminus \{ (0,0) \} \to \R$ such that 
$df = \omega$.  In
\exampleref{example:mv:irrotoneformint},
...
or in other words if there existed an $f$ such that
$df = \omega$.
\end{example}

\subsection{Vector fields}

If at each point we have a vector, what we call a vector field,
we can integrate its component along a path.
Such an integral computes, for example, the work done by a force field
on a particle travelling along a path.
...
the path integral of the vector field $v$ as
...
\begin{exercise}
Show that if $U \subset \R^n$ is a star-shaped domain,
then it is path connected.
\end{exercise}

\begin{exercise}
Show that $U \coloneqq \R^2 \setminus \{ (x,y) \in \R^2 : x \leq 0, y=0 \}$ is
a star-shaped domain and find all points $(x_0,y_0) \in U$ such that
...
Hint: Start with a
point $x \in U$, and let $U_x \subset U$ be the set of points that are
...
connected.  Hint: For contradiction assume there exist two disjoint nonempty open
sets whose union is $U$ and then assume there is a piecewise smooth (and therefore continuous)
...
\end{equation*}
for some integer $\ell$ with $\sabs{\ell} \leq k$.\\
...
times $\gamma$ winds around the origin in the counterclockwise direction.
\end{exercise}
