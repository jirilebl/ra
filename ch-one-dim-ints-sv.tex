...
$\bnorm{\gamma^{\:\prime}(t)}$ without any confusion.
...
\int_a^b f\bigl( \gamma(t) \bigr) \bnorm{\gamma^{\:\prime}(t)} \, dt .
\end{equation*}
...
& =
\int_a^b 
f\bigl(\gamma(t)\bigr) \bnorm{\gamma^{\:\prime}(t)} \, dt
\\
& =
\int_c^d 
f\Bigl(\gamma\bigl(h(\tau)\bigr)\Bigr)
\bnorm{\gamma^{\:\prime}\bigl(h(\tau)\bigr)} h'(\tau) \, d\tau
\\
& =
\int_c^d 
f\Bigl(\gamma\bigl(h(\tau)\bigr)\Bigr)
...
\ell(\Gamma) = 
\int_a^b
\bnorm{\gamma^{\:\prime}(t)}\, dt .
\end{equation*}
This may be a good time to mention that it is common to write
$\int_a^b
\bnorm{\gamma^{\:\prime}(t)}\, dt$ even if the path is only piecewise smooth.
That is because $\bnorm{\gamma^{\:\prime}(t)}$ is defined and continuous
...
\int_0^t
\bnorm{\gamma^{\:\prime}(\tau)}
\, d\tau
\end{equation*}
for all $t$.
The fundamental theorem of calculus then says
that $\bnorm{\gamma^{\:\prime}(t)} = 1$ for all $t$.
Similarly for piecewise smooth $\gamma$, we get
$\bnorm{\gamma^{\:\prime}(t)} = 1$ for all $t$ where the derivative exists.
