...
\volIref{\propref*{vI-prop:integralcontcont} from volume I}
says that $g$ is continuous on $[c,d]$.

Now suppose $f$ is differentiable in $y$.  The main question we want to
...
$\frac{\partial f}{\partial y}$ is integrable in $x$ for every $y$.
...
same.  If you wish you can replace $h$ with a sequence of nonzero 
number $\{ h_n \}$
converging to 0 and let $n \to \infty$.
...
\volIref{\propref*{vI-prop:integralcontcont} from volume I} mentioned above.
\end{proof}
...
for $(x,y) \in [0,1] \times [0,1]$ (see the same exercise).
Therefore
...
\begin{enumerate}[a)]
\item
Prove $f$ is continuous on $[0,1] \times [a,b]$ for any interval
$[a,b]$. Therefore the following function is well defined on $[a,b]$
\begin{equation*}
g(y) := \int_0^1 f(x,y) ~ dx .
\end{equation*}
\item
Prove $\frac{\partial f}{\partial y}$ exists for all $(x,y)$ in
$[0,1] \times [a,b]$, but is not continuous.
\item
Show that $\int_0^1 \frac{\partial f}{\partial y}(x,y) ~ dx$ does not
...
The function $\gamma \colon [a,b] \to \R^n$ is a
...
A path $\gamma$ is a \emph{\myindex{simple path}} if $\gamma|_{(a,b)}$ is
...
then as we have seen, we can treat
$\gamma^{\:\prime}(t)$ either as a matrix (a linear operator) or
a vector as it is an $n \times 1$ matrix, that is,
a column vector.
By an earlier exercise, even the operator norm of
$\gamma^{\:\prime}(t)$ is equal to
the euclidean norm of the corresponding vector.
Then $\gamma^{\:\prime}(t)$ is a vector of the derivatives of the components
of $\gamma$.  If $\gamma(t) =
...
One can mostly get by with smooth paths, but for computations, the simplest
...
The reader can check that the path is the unit square traversed
counterclockwise.  See \figureref{fig:squarepath}.  For example,
...
$\gamma^{\:\prime}(1)$ does not exist.  That is, at the corners $\gamma$ is 
not differentiable. The restrictions are differentiable and the
derivative is different for each restriction.
\end{example}
...
define a so-called \emph{\myindex{one-form}}\index{differential one-form}:
\glsadd{not:oneform}
\begin{equation*}
...
%\omega_n\bigl(\gamma(h(\tau))\bigr) \gamma_n^{\:\prime}(h(\tau)) \right)
%h'(\tau) ~ d\tau 
\\
& =
\int_c^d 
...
integral introducing a minus sign.
...
is, in which direction we traverse the curve, or in other words where we start and
where we finish, then we can just write
\begin{equation*}
\int_{\Gamma} \omega ,
\end{equation*}
...
\emph{\myindex{arc-length measure}}.
...
\end{equation*}
which is what we wanted: $df = \omega$.  As $\omega_j$ is
continuous for every $j$, then $f$ has continuous partial derivatives and
is thus continuously differentiable.
...
path independence, on in other words an
\emph{\myindex{antiderivative}}
$f$ whose total derivative is the given one-form
$\omega$.  Since the criterion is local, we only get the result
...
into any other path
between those two points.  But to make matters simple, the usual way
