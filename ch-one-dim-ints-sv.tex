...
First question is, is $g$ is continuous?
We are really asking when do two limiting operations commute.
As we have seen this is not always possible, so some extra hypothesis
is necessary.
A useful sufficient (but not
necessary) condition is that $f$ is continuous up to the end points.

FIXME: this really belongs in 7.5 as application of uniform continuity
perhaps.  It might give a nice application, though it makes 7.5.9 part a) easier,
though perhaps it was quite challenging before, and main part is part b).

FIXME: exercise 7.5.9 overlaps with 8.1.6, but perhaps that's ok.
If we add the proposition to 7.5, then
8.1.6 should be ``unmarked'' as challenging.
...
As $f$ is continuous on $[a,b] \times [c,d]$ which is compact it
is uniformly continuous.  In particular, there exists $\delta > 0$ such that
whenever $y+h \in [c,d]$ with
$\abs{(y+h)-y} = \abs{h} < \delta$ and all $x \in [a,b]$ we have
$\abs{f(x,y+h)-f(x,y)} < \epsilon$.
In particular,
\begin{equation*}
x \mapsto f(x,y+h)
\qquad
...
As we have seen, this is not always possible, some extra hypothesis is
...
counterclockwise.  See \figureref{fig:squarepath}.  We can check that for example
