...
Let $X$ be the unit sphere in $\R^3$,
that is $X := \{ x \in \R^3 : x_1^2+x_2^2+x_3^2 = 1 \}$.
Take $x$ and $y$ in $X$, and draw a line through the origin and $x$,
and another line through the origin and $y$,
and let $\theta$ be the angle that the two lines make.
Then define $d(x,y) := \theta$.
We can even find a formula with a little bit of vector calculus:
$d(x,y) = \cos^{-1}(x_1y_1+x_2y_2+x_3y_3)$.
