...
\begin{proof}
Suppose $V \subset X$ is open and $U = V \cap Y$.
Let $x \in V \cap Y$.
As $V$ is open, there
...
The set $\R$ is not compact.  Proof: Take the sets $U_j \coloneqq (-j,j)$.
...
For example, let $K \coloneqq [-10,10]$ and for $n \in \Z$
let $U_n \coloneqq (n,n+2)$ define sets in an open cover.
...
\nicefrac{1}{2}$.  If instead we let
$U'_n \coloneqq \bigl(\frac{n}{2},\frac{n+2}{2} \bigr)$,
then we again obtain an open
...
Consider the function defined by $f(x,y) \coloneqq \frac{xy}{x^2+y^2}$
...
\pagebreak[2]
Let $f \colon \R^2 \to \R$ be defined by $f(0,0) \coloneqq 0$, and
