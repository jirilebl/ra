...
of $\{ x_n \}$.  If the limit is unique, We write\glsadd{not:limseq}
\begin{equation*}
...
converging to $c$, the sequence $\bigl\{ f(x_n) \bigr\}$ converges
to $f(c)$.
\end{prop}
...
$\bigl\{ g(p_n) \bigr\}$ converges to $g(p)$ and
$\bigl\{ h(p_n) \bigr\}$ converges to $h(p)$.
\end{example}
...
\}$ is a sequence in $(0,1)$ converging to $0$.  Show that
...
\begin{proof}
Pick $x_0 \in X$.
Define a sequence $\{ x_n \}$ by $x_{n+1} := f(x_n)$.  Then
\begin{equation*}
