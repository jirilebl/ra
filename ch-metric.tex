...
\begin{example}
Suppose $A:=(0,1]$ and $X := \R$.  Then $\widebar{A}=[0,1]$, $A^\circ = (0,1)$,
and $\partial A = \{ 0, 1 \}$.
\end{example}
...
the sequence and use the notation
\glsadd{not:sequence}
\begin{equation*}
\{ x_n \}_{n=1}^\infty .
\end{equation*}
...
$\{ x_n \}_{n=1}^\infty$ converges to $x \in X$ if and only if for every open neighborhood
$U$ of $x$, there exists an $M \in \N$ such that for all $n \geq M$,
we have $x_n \in U$.
\end{prop}

\begin{proof}
Suppose $\{ x_n \}_{n=1}^\infty$ converges to $x$.  Let $U$ be an open neighborhood
...
and $\{ x_n \}_{n=1}^\infty$ a sequence in $E$ that converges to some $x \in X$.
Then $x \in E$.
\end{prop}

\begin{proof}
Let us prove the contrapositive.
Suppose $\{ x_n \}_{n=1}^\infty$ is a sequence in $X$ that converges to $x \in E^c$.
...
Let $(X,d)$ be a metric space and $A \subset X$.
Then $x \in \widebar{A}$ if and only if there exists a sequence
$\{ x_n \}_{n=1}^\infty$ of
elements in $A$ such that $\lim\limits_{n\to\infty} x_n = x$.
\end{prop}
...
a point $x_n \in B(x,\nicefrac{1}{n}) \cap A$.
As $d(x,x_n) < \nicefrac{1}{n}$, we have $\lim_{n\to\infty} x_n = x$.
...
such that there exists a sequence $\{ x_n \}_{n=1}^\infty$ in $A$ that converges to $x$.
...
\begin{enumerate}[a)]
\item
Show that $d(x,y) := \min \bigl\{ 1, \abs{x-y} \bigr\}$ defines a metric on $\R$.
\item
...
there exists a sequence $\{ x_n \}_{n=1}^\infty$ in $S$ that converges to $x$.  Prove
...
Let $d'(x,y) := \sum_{\ell=1}^n \abs{x_\ell-y_\ell}$ and
$d''(x,y) := \max \{ \abs{x_1-y_1},\abs{x_2-y_2},\cdots,\abs{x_n-y_n} \}$.
\begin{enumerate}[a)]
\item
...
As we saw above, a subspace of a complete metric space need not be
complete.  However, 
...
so no subsequence of $\{ x_n \}_{n=1}^\infty$ can be
Cauchy.  Hence, no subsequence of $\{ x_n \}_{n=1}^\infty$ can be convergent,
...
every sequence $\{ x_k \}_{k=1}^\infty$ in $K$ is bounded, so it has a convergent
subsequence by
...
Consider the function defined by $f(x,y) := \frac{xy}{x^2+y^2}$
outside the origin and $f(0,0) := 0$.
...
\begin{proof}
Fix $y \in [c,d]$, and let $\{ y_n \}_{n=1}^\infty$ be a sequence in $[c,d]$
converging to $y$.
...
\begin{enumerate}[a)]
\item
Show that $T(f) := \varphi_f$ defines a function
...
\begin{equation*}
d(f,g) := \snorm{f-g}_X = \sup_{x \in X} \abs{f(x)-g(x)} .
\end{equation*}
\begin{enumerate}[a)]
\item
...
Define
\begin{equation*}
d_{C}(f,g) := \snorm{f-g}_{[a,b]}
\qquad \text{and} \qquad
d_{C^1}(f,g) := \snorm{f-g}_{[a,b]} + \snorm{f'-g'}_{[a,b]},
\end{equation*}
...
Given a map $f \colon X \to X$, a point $x \in X$ is called a
...
\begin{proof}
Pick $x_0 \in X$.
Define a sequence $\{ x_n \}_{n=1}^\infty$ by $x_{n+1} := f(x_n)$.  Then
\begin{equation*}
...
we let $x := \lim_{n\to\infty} x_n$, and we claim that $x$
...
fixed point theorem.
That is, the
space $C\bigl([a,b],\R\bigr)$ of \exampleref{example:msC01},
...
It may be strange that we are looking in $C\bigl([a,b],\R\bigr)$ for a differentiable
...
\begin{equation*}
Y := \bigl\{ f \in C\bigl([-h,h],\R\bigr) : f\bigl([-h,h]\bigr) \subset J \bigr\} .
\end{equation*}
