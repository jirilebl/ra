...
\begin{enumerate}[a)]
\item
Find a metric $d$ on $\N$ that makes it into an unbounded metric space.
\item
Find a metric $d$ on $\N$ that makes it into a bounded metric space.
\item
...
\begin{enumerate}[a)]
\item
Prove the Cauchy--Schwarz inequality for two sequences
...
the main features of an interval in $\R$ is that it is connected,
that we can continuously move from one point of it to another point without
jumping.  On the real numbers we usually study functions on intervals,
and in more general metric spaces we often study functions on connected sets.
...
\item
Prove that if $X = \R$, then no matter what the metric is, there exists
