...
real numbers.  In \chapterref{lim:chapter}, we learned to take limits
of functions as a real variable approached some real number.
...
%mbxENDIGNORE
%mbxlatex $d(x,y) = 0$ if and only if $x = y$. \qquad
...
Items \ref{metric:pos}--\ref{metric:com} have an obvious geometric
...
A triangle with three vertices x, y, and z.
The distance between x and z is marked as
d of x comma z, the distance from x to y
is marked as d of x comma y, and the distance
from y to z is marked as d of y comma z.
An arrow from x to z is labeled "shorter", and
an arrow that goes from x to y and then turns to z
is labeled "longer".}
...
Note that $d(x,y) = \varphi\bigl(\sabs{x-y}\bigr)$ where $\varphi(t) =
...
A graph of a function that starts going diagonally
up from the horizontal axis and then slowly turns
so that it becomes almost horizontal at the right
end of the graph and a horizontal asymptote above
the graph is marked with a dashed line.}
...
\begin{example}
An example to keep in mind is the
...
A diagram of 5 points a, b, c, d, and e.  Each point
is connected to each other point with a line and each line is labeled
with the number 1.}
...
then $\babs{f(x)-g(x)} = 0$ for all $x$, and hence $d(f,g) = 0$.  Conversely,
if $d(f,g) = 0$, then for every $x$, we have $\babs{f(x)-g(x)} \leq d(f,g) = 0$,
...
Notice that $d(f,g) = \snorm{f-g}_{[a,b]}$, the uniform norm of \defnref{def:unifnorm}.
...
The triangle inequality is harder to prove, and requires a bit more
...
A diagram of a sphere in 3-dimensional space
that is marked S superscript 2.  The origin is marked as 0,
and there are two points on the sphere, x and y.  There is
a line between the origin and x, a line between the origin and y,
and the angle between these two lines is labeled with a theta.}
...
which are left as exercises.  Suppose $X$ is nonempty to avoid a technicality.
...
Prove that
$(Y,d_H)$ is what we call a \emph{\myindex{pseudometric space}}:
...
\item
Show by example that $d$ itself is not symmetric,
that is, $d(A,B) \neq d(B,A)$.
\item
...
A blob in the plane marked in light gray,
with a dotted boundary and labeled as V.
Inside the blob there is a dotted
circle with center
at a point marked x of radius delta and labeled
as B of x comma delta.  The circle and its inside
is shaded darker gray and lies completely inside the
lighter gray blob.}
...
Two shaded blobs with a solid boundary are both
...
each shaded blob is inside only one of the dotted
lines.}
...
there is a point z and w.  An open interval
...
\begin{myfigureht}
\myincludepdft{compact_bnd_closed}{FIXMEALT}
...
\begin{myfigureht}
\myincludepdft{lebesguedelta}{FIXMEALT}
...
\begin{myfigureht}
\myincludepdft{seqcompactiscompact}{FIXMEALT}
...
\begin{myfigureht}
\myincludepdft{mscontfuncpt}{FIXMEALT}
