...
%Let $\alpha := \inf \, S$ and
%$\beta := \sup \, S$, and note that $\alpha < \beta$.  Suppose $\alpha < z <
%\beta$ for some $z \in \R$.  As $\alpha$ is the
...
%As $U_1$ and $U_2$ were arbitrary, $S$ is connected.
\end{proof}
...
\emph{\myindex{completely disconnected}}, that is, show 
...
a)~Is $\overline{A}$ connected?  Prove or find a counterexample.\\
b)~Is $A^\circ$ connected?  Prove or find a counterexample.\\
...
\\
b) Find an explicit example of $(X,d)$, $X$ infinite, where
...
\}$.\\
a) Use \exerciseref{exercise:mscross}, to show that
$(\R^n,d')$ and
$(\R^n,d'')$ are metric spaces.\\
b)
Let $\{ x_j \}_{j=1}^\infty$ be a sequence in $\R^n$ and $p \in \R^n$.
Prove that the following statements are equivalent:\\
(1) $\{ x_j \}$ converges to $p$ in $(\R^n,d)$.\\
(2) $\{ x_j \}$ converges to $p$ in $(\R^n,d')$.\\
(3) $\{ x_j \}$ converges to $p$ in $(\R^n,d'')$.
\end{exercise}
...
% is compact by proving that in $\R$, a closed and bounded set is compact.
\end{example}
...
But there are many complete and very useful metric spaces
...
space, but the closed unit ball $C(0,1)$ is not compact.  However, see also
...
$n_j := k$.
We now posses a subsequence $\{ x_{n_k} \}$.
Since
...
a)~Prove that $X$ is complete.\\
b)~Prove that $X$ is compact if and only if $X$ is a finite set.
\end{exercise}
...
\bigr\rvert$.\\
a) Show that $(\R,d')$ is a metric space (if you have done
\exerciseref{exercise:extendedrealsmetric} the computation is the same).\\
b) Show that the topology is the same, that is, a set is open in
$(\R,d)$ if and only if it is open in $(\R,d')$.\\
c) Show that a set is compact in
$(\R,d)$ if and only if it is compact in $(\R,d')$.\\
d) Show that a sequence converges in $(\R,d)$ if and only if
it converges in $(\R,d')$.\\
e) Find a sequence of real numbers that is Cauchy
in $(\R,d')$ but not Cauchy in $(\R,d)$.\\
f) While $(\R,d)$ is complete, show that $(\R,d')$ is not complete.
\end{exercise}

\begin{exercise}
Let $(X,d)$ be a metric space and $K \subset X$ a set.
Prove that $K$ is compact as a subset of $(X,d)$ if and only if it is
...
(differentiable by \thmref{thm:FTCv2} and
unique by \thmref{thm:FTCv1}).  Therefore $f$ is the unique
