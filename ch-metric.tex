...
We avoid the empty set for essentially the same reason that
we do not consider 1 a prime number.}
...
$A$.  There is at least one closed set containing every $A$, namely $X$
itself.  So $A \subset \overline{A}$.
...
We can also talk about what is in the interior of a set
(points we cannot approach from the complement),
and what is on the boundary (points we can
