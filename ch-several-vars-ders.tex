...
$t=0$.\\
Hint: While not necessary \volIref{\S 4.3 of volume I} makes
this part easier.
\item
...
Pick two nonzero numbers $s$ and $t$ small enough so that
$p+s_0e_j +t_0e_k \in U$ for all $s_0$ and $t_0$ with
$\sabs{s_0} \leq \sabs{s}$ and
$\sabs{t_0} \leq \sabs{t}$.  This can be done as $U$ is open and so
...
\end{equation*}
Now reverse the ordering, starting with
the function $f(p+te_k)-f(p)$ find an $s_1$ between $0$ and $s$ such that
\begin{equation*}
...
\end{equation*}
Find a $t_1$ between $0$ and $t$ such that
\begin{equation*}
