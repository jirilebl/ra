...
so on.  
One particular fact we will often use is that $0 v = 0$, where
the zero on the left is $0 \in \R$ and the zero on the right is $0 \in X$.
To see this start with
$0v = (0+0)v = 0v + 0v$, and add $-(0v)$ to both sides
to obtain $0 = 0v$.  Similarly $-v = -1v$, which follows by
$-1v+v = -1v +1v = (-1+1)v = 0v = 0$.
...
is a vectorspace.
That is, if $X$ is a vectorspace and
...
\sabs{\lambda} \, \snorm{y}^2 = \snorm{\lambda y} \snorm{y} = \snorm{x}
\snorm{y} .
\end{equation*}
...
It is also not difficult to compute the norm of the identity operator:
\begin{equation*}
\snorm{I} =
...
\begin{equation*}
\snorm{(cA)x} =
\sabs{c} \snorm{Ax} \leq (\sabs{c}\snorm{A}) \snorm{x} .
\end{equation*}
...
numbers $a$ and the operator norm of $a$ is simply $\sabs{a}$.
...
The proof of the second part is rather easy.  Take $f(x) e_j$, which
...
$\sabs{\det(A)}$.  The sign of the determinant tells us if the image is
...
then $\sabs{c_j} \leq B$.
\item
Use part (c) to show that if $X$ and $Y$ are finite dimensional vector 
...
\begin{equation*}
\lim_{h \to 0} \sabs{\frac{f(x+h)-f(x)}{h} - a} =
\lim_{h \to 0} \sabs{\frac{f(x+h)-f(x) - ah}{h}}
=
...
%$\sabs{t} \leq \snorm{h}$ and $\snorm{h_1} \leq \snorm{h}$.
\end{proof}
