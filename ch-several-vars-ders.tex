...
So any subspace $V$ of a vector space $X$ is convex.
\end{example}
...
\emph{\myindex{convex hull}}.  Given any subset $S \subset V$ of a vector
...
\qquad (Triangle inequality)\index{triangle inequality!norms}.
\end{enumerate}
...
are changed.
Item \ref{prop:det:iv} follows because if two columns are equal and we switch them we get
the same matrix back and so item \ref{prop:det:iii} says the determinant must have been
0.
...
\begin{defn}
Let $U \subset \R^n$ be an open subset and $f \colon U \to \R^m$ a function.  We
...
\item
For every direction the restriction of $f$ to a line through the origin
...
It is the zero set of the function
\begin{equation*}
