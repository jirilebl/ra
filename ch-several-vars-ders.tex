...
$\{ \widetilde{x}_1,\widetilde{x}_2,\ldots,\widetilde{x}_\ell \}$.  We can write
...
$y_j \coloneqq A x_j$.  Every $x \in X$ has a unique representation
\begin{equation*}
...
\sum_{j=1}^n b_j \, y_j .
\end{equation*}
The \myquote{furthermore} follows by setting $y_j \coloneqq \widetilde{A}(x_j)$,
and then for 
$x = \sum_{j=1}^n b_j \, x_j$,
defining the extension as
$Ax \coloneqq \sum_{j=1}^n b_j \, y_j$.  The function is well-defined by
...
$Ax_j \coloneqq e_j$, where $\{ e_1, e_2, \ldots, e_n \}$ is the standard
...
function
\begin{equation*}
Ax \coloneqq \sum_{j=1}^n b_j y_j, \qquad \text{if} \quad x=\sum_{j=1}^n b_j x_j .
\end{equation*}
...
\begin{equation*}
x \cdot y \coloneqq \sum_{j=1}^n x_j\, y_j .
\end{equation*}
...
For $x \in \R^n$, define
\begin{equation*}
\snorm{x}_{1} \coloneqq \sum_{j=1}^n \sabs{x_j},
\end{equation*}
