...
In this book, we mostly deal with vector spaces that can often be regarded as
subsets of $\R^n$, but other vector spaces are useful in
analysis.  Let us give a few examples.
...
\end{enumerate}
Items 2) and 3)
make sure that addition and scalar multiplication are indeed defined on~$S$.
Item 1) is required
...
It is better to think of even the simpler \myquote{finite-dimensional}
...
\{ \widetilde{x}_1,\widetilde{x}_2,\ldots,\widetilde{x}_n \}$, by just taking zero
multiples of the vectors we do not need, e.g. $x_1 = x_1 + 0 \widetilde{x}_1$.
So if we have two linear combinations, we can without loss of
generality write them as a linear combination of one subset of vectors
$\{ x_1,x_2,\ldots,x_m \}$ from~$Y$.
...
the \emph{\myindex{dimension}} of $X$ is $d$,
and we write $\dim \, X \coloneqq d$.
If for all $d \in \N$ the vector space
...
So notice that if a set does not have $d+1$
...
$T \coloneqq \{ y_1, y_2, \ldots, y_m \}$ is a set of linearly independent
...
Suppose without loss of generality that $a_{1,1} \not= 0$.  We solve
\begin{equation*}
...
transformation $AB$ as the composition $A \circ B$,
\begin{equation*}
ABx \coloneqq A(Bx) .
\end{equation*}
...
$Ax \coloneqq \sum_{k=1}^n b_k \, y_k$.  The function is well-defined by
...
and hence $c_k = 0$ for all $k$.  In other words, if $Ax = 0$, then $x=0$.  This means that
...
this, take any $x \in \R^2 \setminus \{0\}$ and let $y \coloneqq -x$.
Then $(\nicefrac{1}{2})x + (\nicefrac{1}{2})y = 0$, which is not in the set.
Balls (in the standard metric) in $\R^n$ are convex.  We use this result often enough we state
...
$f(x) \coloneqq 1$ is in $X$, but $2f$ and $-f$ is not.
\end{example}
...
function
\begin{equation*}
Ax \coloneqq \sum_{k=1}^n b_k y_k, \qquad \text{if} \quad x=\sum_{k=1}^n b_k x_k .
\end{equation*}
...
we define a distance $d(x,y) \coloneqq \snorm{x-y}$, and this
...
then $c_k = x \cdot e_k$ and by Cauchy--Schwarz
\begin{equation*}
...
indeed implies that $b$ is not zero, and $a \mapsto \nicefrac{1}{a}$
...
\begin{enumerate}[a)]
\item
Prove that there exist positive numbers $m,M > 0$ such that
for all $c \in \R^n$,
\begin{equation*}
m \snorm{c}
\leq
