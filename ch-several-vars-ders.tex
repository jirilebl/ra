...
In $\R^n$ we often think of vectors as direction and magnitude, and draw the
...
not based at the origin necessarily; a vector is then simply the direction and
...
It is often better to think of even the simpler ``finite dimensional''
...
on $\R^n$)
is convex by the triangle inequality (exercise).
...
Let $GL(X) \subset L(X)$\glsadd{not:GLX} be the set of invertible linear operators.
\begin{enumerate}[(i)]
...
Define the symbol
$\operatorname{sgn}(x)$\glsadd{not:sgn} (read ``sign of $x$'') for a number $x$ by
\begin{equation*}
\operatorname{sgn}(x)
:=
\begin{cases}
...
it can be shown (exercise) that $\operatorname{sgn}(\sigma)$ 
...
Then note that $\operatorname{sgn}(1,2,\ldots,n) = 1$.
...
the 
same determinant as if we had used any other basis: in the other basis
the matrix would be $B^{-1}AB$.
It follows that
\begin{equation*}
\det \colon L(X) \to \R
\end{equation*}
...
\begin{exercise}
Consider \propref{prop:finitedimpropinv} and let $A$ be invertible.
\begin{enumerate}[a)]
\item
...
A (possibly complex) number $\lambda$ is called an \emph{\myindex{eigenvalue}} of $A$
if there is a nonzero (possibly complex) vector $x$ such that $Ax = \lambda
x$ (the multiplication by complex vectors is the same. In particular if $x =
...
For convenience,
we often 
leave out the points where we are evaluating, such as above on the right hand side.
...
\begin{exercise}
Suppose $g \colon \R^n \to L(\R^n,\R)$ is a continuous function.  Suppose
that $f \colon \R^n \to \R$ and $h \colon \R^n \to \R$ are two 
differentiable functions such that $f'(x) = g(x) = h'(x)$.  Prove that
