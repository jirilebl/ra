...
In $\R^n$ we often think of vectors as a direction and a magnitude, and draw the
...
There is a unique $x_1 \in V$ such that $f(x_1) = y_1$.
...
%Recall $\varphi_y(x) = x+A^{-1}\bigl(y-f(x)\bigr)$.
If $x \in
C(x_1,r)$, then
\begin{equation*}
\begin{split}
...
\end{equation*}
Furthermore, $Bx$ is the unique $y \in \R^m$ such that $A(x,y) = 0$.
\end{prop}
...
That is, there exists an open set $V \subset \R^{n+m}$ with $(p,0) \in V$,
and a 
mapping $G \colon V \to \R^{n+m}$, such that $F\bigl(G(x,s)\bigr) = (x,s)$ for
all $(x,s) \in V$. % (where $x \in \R^n$ and $s \in \R^m$).
...
The set $G(V)$ contains a whole neighborhood of the point $(p,q)$.
Hence there exist some open sets
$\widetilde{W}$ and $W'$ such that $\widetilde{W} \times W' \subset G(V)$ with $p
\in \widetilde{W}$ and
$q \in W'$.
Take $W := \{ x \in \widetilde{W} : G_2(x,0) \in W' \}$.
...
\end{equation*}
at $p$,
which should be the zero map.
%The derivative is done in the same way as above.
Applying the chain rule,
we get that for all $h \in \R^{n}$
\begin{equation*}
