...
We write $L(X,Y)$\glsadd{not:LXY} for the set of linear transformations from $X$ to
...
the set $\R^2 \setminus \{0\}$ is connected, but not convex.  To see
this, take any $x \in \R^2 \setminus \{0\}$ and let $y \coloneqq -x$.
Then $(\nicefrac{1}{2})x + (\nicefrac{1}{2})y = 0$, which is not in the set.
Balls (in the standard metric) in $\R^n$ are convex.  We use this result
often enough to state it as a proposition,
and we leave its proof as an exercise.
