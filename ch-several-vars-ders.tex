...
functions
(approximating the graph by a flat object such as a line or a plane).
...
not necessarily based at the origin; a vector is simply the direction and
...
On the left a diagram of the vector (v sub 1, v sub 2)
in the x sub 1 x sub 2 plane as an arrow from the origin
to the point (v sub 1, v sub 2).
On the right a diagram of vector addition and scalar
multiplication.
That is, first an image of an arrow v followed by
a vector w placed at its head, and then the vector
v plus w that starts at the tail of v and ends at the head of w.
Then an image of an arrow v and an arrow 2 v, where 
the arrow 2 v is in the same direction but twice
as long as v.}
...
it as an arrow if we think of the \emph{vector space structure} on
...
and multiplication by a scalar are done componentwise:
...
vector spaces abstractly rather than as $\R^n$.
...
\begin{prop}
Let $X$ be a vector space and $Y \subset X$ a subset.
...
Suppose $d$ is the largest integer for which $X$ contains a set of
...
independent.
So $X$ is of dimension $d$ if there
...
$a_{1,2},a_{2,2},\ldots,a_{d,2}$, such that
\begin{equation*}
...
If $a_{m+1} \neq 0$, then $v$ would be a linear combination of $T$,
...
Let $a \in \R$ and $y \in Y$.  As $A$ is onto, there is an 
...
An oval shaded region labeled U.  Two points,
x and y, are marked inside and a line segment between them, which lies
entirely inside U.  A point somewhere in the middle of the segment
is labeled as 1 minus t all times x plus t times y.}
...
$f(x) \coloneqq 1$ is in $V$, but $2f$ and $-f$ are not.
\end{example}
...
\end{equation*}
We will normally write $\snorm{x}$.
Only in the rare instance when it is necessary to
...
\end{equation*}
This implies, assuming $\snorm{A} \neq \infty$ to avoid a technicality when $x=0$,
...
The operator norm is not always a norm on $L(X,Y)$.
In particular,
...
\end{equation*}
As $\snorm{A} < \infty$, the inequality above says that
...
whenever $a \neq 0$.  The condition $\sabs{a-b} < \frac{1}{\sabs{a^{-1}}}$
...
Fix an $A \in GL(X)$.  Let $B$ be near $A$,
...
The same can be done with $L(X,Y)$, bringing us to matrices,
...
original ordering obtains the sgn.
...
\begin{prop}
The determinant is independent of the basis:  If $A$ and $B$ are
...
Such a picture is called \emph{\myindex{commutative diagram}}.
In a commutative diagram,
following arrows in any way must end up with the same result.}:
%mbxSTARTIGNORE
\begin{equation*}
...
%mbxlatex \myincludepdft{change_of_basis_cd}{A diagram of sets
%mbxlatex and mappings between them represented by arrows.
%mbxlatex First row has two sets, both the n dimensional euclidean space, R super n.
%mbxlatex Second row has two sets, both X.
%mbxlatex Third row has two sets, both the n dimensional euclidean space, R super n.
%mbxlatex The top left R super n has an arrow labeled B inverse A B
%mbxlatex to the top right R super n.
%mbxlatex The top left R super n has an arrow labeled B to the R super n on the bottom left
%mbxlatex The bottom right R super n has an arrow labeled B inverse from the R super n
%mbxlatex on the bottom right.
%mbxlatex The left X has an arrow labeled Psi
%mbxlatex to the top left R super n,
%mbxlatex an arrow labeled Phi to the bottom left R super n,
%mbxlatex and an arrow labeled T to the right X.
%mbxlatex The top right R super n has an arrow labeled
%mbxlatex Psi inverse to the right X, and the
%mbxlatex bottom right R super n has an arrow
%mbxlatex labeled Phi inverse to the right X.
%mbxlatex The bottom left R super n has an arrow labeled A
%mbxlatex to the bottom right R super n.}
%mbxlatex \end{center}
...
There are three types of what we call
...
\neq j$.  Show that $\det(D) = d_{1,1}d_{2,2} \cdots d_{n,n}$.
\item
...
symmetric convex set that spans $\R^n$ and $0 \in C$.  Show that
\begin{equation*}
...
\end{enumerate}
Hint: Feel free to use the result of \exerciseref{exercise:allnormsequiv}
part c).  In particular, whether a set is \myquote{compact} is independent of the
norm.
\end{exercise}
...
A graph of a sample function y equals f of x sub 1, x sub 2.
A particular point is marked on the x sub 1 x sub 2 plane.
There is an arrow labeled h in the x sub 1 x sub 2 plane.
On the graph a piece of the tangent plane (that is, a plane tangent to the
graph) is shown above the point marked.  A horizontal line representing h
is drawn starting on the graph now.  Then from the end of that line 
a vertical arrow that moves us to the tangent plane is drawn and
labeled A h.}
...
it is not always necessary to explicitly say so.
...
A graph of a sample function y equals f of x sub 1, x sub 2.
A particular point is marked on the x sub 1 x sub 2 plane.
A plane in the x sub 2 and y direction is marked in a dotted line
and the intersection of the graph of the function f with this plane
is marked in thicker line.  The point on the graph above the
given x sub 1 x sub 2 point is also marked, through this point
and inside the dotted plane the tangent line to the graph is drawn.
It is labeled with slope equals the partial derivative of f with
respect to x sub 2 at the point x sub 1, x sub 2.}
...
We treat $\gamma'(t)$ both as an operator in
$L(\R^1,\R^n)$ and the vector
$\bigl(\gamma_1'(t),\gamma_2'(t),\ldots,\gamma_n'(t)\bigr)$
in $\R^n$.
Using \propref{mv:prop:jacobianmatrix},
if $v\in \R^n$ is $\gamma'(t)$ acting as a vector,
then $h \mapsto h \, v$ (for $h \in \R^1 = \R$) is
$\gamma'(t)$ acting as an operator
in $L(\R^1,\R^n)$.
We often use this 
slight abuse of notation when dealing with curves.
The vector $\gamma'(t)$ is called a \emph{\myindex{tangent vector}}.
See \figureref{fig:difcurveder}.
\begin{myfigureht}
\myincludepdft{diffcurveder}{%
A curve in the plane is drawn.
The beginning of the curve is marked as gamma of a
and the end is marked gamma of b.  The entire curve
is marked gamma of (a,b).
A point on the curve is marked gamma of t, and an arrow
starting at this point and pointing along the curve is
marked gamma prime of t.}
\caption{Differentiable curve and its derivative as a
vector (for clarity assuming $\gamma$ defined on $[a,b]$).
The tangent vector $\gamma'(t)$ points along the curve.\label{fig:difcurveder}}
\end{myfigureht}
...
is called the
\emph{\myindex{Jacobian matrix}}, or sometimes confusingly also called just
\myquote{the Jacobian.}},
of $f$ at $x$ as
\glsadd{not:jacobdet}
\begin{equation*}
...
determinant
is nonzero, then we would assume that locally the mapping is invertible (and
...
A graph in the xyz-space of a function z equals f of x, y, which is a cone
facing upwards from the origin.  That is, one could construct this
graph by going upwards at 45 degrees along every direction in the xy-plane away from the
origin.}
...
A graph of a function z equals f of x, y.
The function is zero on the x and y axes.
In the first and third quadrant it is positive
and negative in the second and fourth quadrant.
It is bounded, but it is not continuous at the origin
as if we approach along the y equals x the function seems
to be constant and positive, and along the y equals minus x
line the function seems constant and negative.  The graph appears as
if we took a rubber sheet at the origin with one hand from above and
one hand from below and we stretched the origin into kind of stretched
pinch.}
...
A graph of a function z equals f of x, y.
The function is zero on the x and y axes.
In the first and second quadrant it is positive
and negative in the third and fourth quadrant.
While it is obviously continuous, it is clear
from the picture that it will have the same shape
if we keep zooming into the origin, and so it will not
have a tangent plane at the origin.}
...
\begin{exercise}
Prove a version of the \emph{\myindex{mean value theorem}}
...
\begin{exercise}
Define $f \colon \R^2 \to \R$ by $f(x,y) \coloneqq \frac{y(x^2+y^2)}{x}$
when $x \neq 0$ and $f(0,y) \coloneqq 0$.  Show that for all $u \in \R^2$
with $\snorm{u}=1$, we get $D_u f(0) = 0$, that is, the directional
derivative exists and is zero in all directions at the origin, but the
function is not continuous at the origin (in fact, not continuous whenever
$x=0$).
\end{exercise}
...
be a differentiable function, and $M$ be such that
\begin{equation*}
\bnorm{f'(p)} \leq M
...
\end{equation*}
Think of a spiral staircase with room in the middle.  See
\figureref{mv:fignonlip}.

\begin{myfigureht}
\myincludepdft{nonlip_full}{%
On the left, there is a diagram in the xy-plane
which is a shaded washer (the space between two circles) centered at the
origin that is cut along the negative x axis.
A point (x,y) is labeled in the washer and a line from this point
to the origin is drawn, the angle from the positive x-axis
to this line is labeled as theta equals f of x, y.
On the right, there is a graph of this function which is rising spiral
that starts below the xy-plane when we start walking along the washer
from the cut counterclockwise and then it will get up to the xy-plane
when we get to the positive x axis and it will be above the xy-plane
when we get to the cut from the other side.}
...
$\bnorm{f'(y)} \leq 0$ for all $y \in B(x,\delta)$, by the proposition,
...
$M \in \R$, such that
for every $(x,y) \in \R^2$ with $x^2+y^2=1$, the function $g(t) \coloneqq f(xt,yt)$ is
differentiable
...
A diagram of two sets and mappings between them.
On the left is the shaded set U with dotted boundary and a smaller subset V
shaded lightly with a dotted boundary.  Points p and x are marked in V.
On the right is a lightly shaded set labeled W equals f of V with two
points one marked f of p and one marked y.  Two arrows labeled f go from left
to right one going from the point p to the point f of p
and one going from the point x to the point y.  Two arrows labeled g
go the opposite way between the same points.}
...
$\varphi_y(x) = x$ ($x$ is a fixed point) if and only if
...
A diagram of two sets and mappings between them.
On the left is a shaded set V with dotted boundary and
the points x plus h and x marked.
On the right is a shaded set W with the points y plus k
and y marked.
Two arrows labeled f go from left
to right one going from the point x plus h to the point y plus k
and one going from the point x to the point y.  Two arrows labeled g
go the opposite way between the same points.}
...
As $x \in V$, we find that $f'(x)$ is invertible.
...
\begin{example}
The theorem, along with the corollary, is not true if $f'(x)$ is not invertible for
some~$x$.
...
mapping.  The function $f(x) \coloneqq x^3$ is an open mapping from $\R$
to $\R$ and is globally one-to-one with a continuous inverse, although the
inverse is not differentiable at $x=0$.

\medskip

As a side note, there is a famous related, and as yet not completely solved, problem
...
The inverse function theorem gives a local $C^1$ inverse,
but whether one can always
find a global polynomial inverse is the question.
The answer is true for $n=1$, and there is a counterexample
found recently for $n \geq 3$.

\subsection{Implicit function theorem}

The inverse function theorem is a special case of the implicit
function theorem, which we prove next.  Somewhat ironically we 
prove the implicit function theorem using the inverse function theorem.
One way to state the inverse function theorem is that
...
The proof is immediate: We solve $A_x x +A_y y = 0$ and obtain $y = Bx$.
Another approach is to \myquote{complete the basis,} that is, add
...
is $\frac{\partial f}{\partial y}(p,q) \neq 0$, and $W$ and $W'$ are 
open intervals.  See \figureref{fig:implicitfunc}.
\begin{myfigureht}
\myincludepdft{implicitfunc}{%
A diagram of a circle in the xy-plane labeled f of x,y equals 0.
A point labeled (p,q) is marked on the circle in the first quadrant.
It is contained in a small shaded rectangle labeled W cross W prime.
The projection of the shaded rectangle onto the x and y axes are
labeled W and W prime respectively and both fit between the origin
and where the circle hits the corresponding axis.}
...
if $D(x_0,y_0) \neq 0$ and $z^2+x_0z+y_0 = 0$ for some $z \in \R$,
...
\item
Show that if $r \neq 0$, then $F'(r,\theta)$ is invertible, therefore an
...
a $C^1$ function such
that $\nabla f(x,y) \neq 0$ for all $(x,y) \in U$.  Show that every
...
If $\frac{\partial f}{\partial x_{\ell}}$ exists everywhere in $U$,
...
\end{equation*}
exists and is differentiable in the variable $x_{{\ell}_{k}}$.
Then the partial derivative with respect to that variable is denoted by
\begin{equation*}
...
a \emph{$k$-times continuously differentiable function}%
...
are continuous.
The existence of lower order partial derivatives is clearly
...
with a point p marked.
A horizontal arrow to the right is labeled with e sub ell,
and a vertical arrow up is labeled with e sub m.
The point p plus s times e sub ell is marked
on the horizontal arrow.
On the vertical arrow, we have a point
p plus t times e sub m and below it
the point 
p plus t sub 0 times e sub m.
Above p plus s e sub ell and to the left of p plus t times e sub m
we have a point p plus s times e sub ell plus t times e sub m.
Above p plus s e sub ell and to the left of p plus t sub 0 times e sub m
we have a point p plus s times e sub ell plus t sub 0 times e sub m.
Finally, between the points
p plus t sub 0 times e sub m and
p plus s times e sub ell plus t sub 0 times e sub m
is the point
p plus s sub 0 times e sub ell plus t sub 0 times e sub m.}
...
Now reverse the roles of $s$ and $t$ (and $\ell$ and $m$).
