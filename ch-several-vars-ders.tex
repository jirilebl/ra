...
Let $X$, $Y$, and $Z$ be normed vector
spaces.
{\ }
...
\end{equation*}
In particular, if $X$ is finite dimensional, the operator norm is a norm on the vector space $L(X,Y)$.
...
If $X$ is finite dimensional, then $\snorm{A} < \infty$ for all $A \in
L(X,Y)$, and therefore the operator norm is a norm.
