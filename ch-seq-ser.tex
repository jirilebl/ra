...
\R$.  Instead of $x(n)$, we 
usually denote the $n$th element in the sequence by $x_n$.  To denote a
sequence we write
\glsadd{not:sequence}%
\begin{equation*}
\{ x_n \}_{n=1}^\infty.
...
Write
$x := \limsup_{n\to\infty} x_n = \lim_{n\to\infty} a_n$.
...
Set $n_{k} :=  m$.  The subsequence $\{ x_{n_k} \}_{k=1}^\infty$ is defined.  Next, we
...
of $\{ a_n \}_{n=1}^\infty$.
However, as $n_k \geq k$ for all $k$, we have that
...
x = \sup \{ a_i : i \in \N \} .
\end{equation*}
Let $y := \lim_{i\to\infty} b_i = \inf \{ b_i : i \in \N \}$.  Since $a_i < b_i$ for
all $i$, then $x \leq y$.
...
\begin{exercise}
Suppose $\{ x_n \}_{n=1}^\infty$ is a bounded sequence, $a_n := \sup \{ x_k : k \geq n \}$
...
suppose $\{ x_n \}_{n=1}^\infty$ converges to $x$.  Then there 
...
Define $a := \limsup_{n\to\infty} x_n$ and
$b := \liminf_{n\to\infty} x_n$.
...
If $x := \lim\limits_{k\to\infty} s_k$, we write
\begin{equation*}
...
\geq -x_1 .
\end{equation*}
The sequence $\{ s_{2k} \}_{k=1}^\infty$ is decreasing and bounded below, so it converges.
Let $a := \lim_{k\to\infty} s_{2k}$.
