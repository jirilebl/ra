...
\frac{\epsilon}{3K}
\frac{\epsilon}{3K}
\qquad \qquad \text{(now notice that $\tfrac{\epsilon}{3K} \leq 1$ and $K \geq 1$)}
\\
& \leq
...
You may have seen the sequence above before.  It is the
...
If we look at the proposition above, we note that the
...
Then for any $N \geq K$, let $Q := \max \sigma(\{ 1,2,\ldots,N \})$
and compute
\begin{equation*}
\begin{split}
...
infinity and minus infinity respectively (prove this!):
\begin{equation*}
...
\href{https://en.wikipedia.org/wiki/Ernesto_Ces\%C3\%A0ro}{Ernesto Ces{\`a}ro}
...
\item
Show that in the sense of Ces{\`a}ro $\sum {(-1)}^n$ is summable to
$\nicefrac{1}{2}$.
\item
