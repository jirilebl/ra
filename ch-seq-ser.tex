...
Another example of a sequence is the \emph{\myindex{constant sequence}}.
...
Two diagrams.
The first is a graph of a sequence, that is,
the values are on the vertical axis and the horizontal axis is the index.
On the vertical axis,
the number x is marked and a horizontal
line is drawn.  A strip from
x minus epsilon to x plus epsilon is shaded.  The values
of the sequence are mostly outside the shaded strip
until the index is 7, which is marked as M.  All the 
subsequent values are within the strip.
On the second diagram, the x-axis is horizontal, with
the number x marked and the values x sub 1, x sub 2, until x sub 10 marked
as ticks.}
...
The first few terms of a sample monotone increasing sequence
are shown in 
\figureref{figsequenceincreasing}.

\begin{myfigureht}
\myincludegraphics{sequence-increasing}{%
A graph of the first few terms of a sequence with the horizontal axis being
the index.  Each subsequent value in the sequence is either the same or
larger than the previous, so the dots giving the values go up as we move
right across the graph.}
...
it is easy to see (by \hyperref[induction:thm]{induction}) that
...
%mbxlatex \myincludepdft{tail_of_sequence_cd}{FIXMEALT}
%mbxlatex \end{center}
...
\begin{myfigureht}
\myincludepdft{figsqueeze}{FIXMEALT}
...
\begin{myfigureht}
\myincludepdft{figratseq}{FIXMEALT}
...
\end{proof}
\begin{myfigureht}
\myincludegraphics{sequence-limsupliminf_an_bn}{FIXMEALT}
...
\begin{myfigureht}
\myincludegraphics{sequence-limsupliminf_an_bn-example}{FIXMEALT}
...
\begin{myfigureht}
\myincludepdft{figcutseries}{FIXMEALT}
...
\begin{myfigureht}
\myincludegraphics{ser-alternate}{FIXMEALT}
...
\begin{myfigureht}
\myincludegraphics{ser-rearrange}{FIXMEALT}
...
\begin{myfigureht}
\myincludepdft{ps-conv}{FIXMEALT}
...
On part f), do note the factorial in the exponent.
