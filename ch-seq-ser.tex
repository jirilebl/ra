...
distinct.  For example, the sequence $\{ {(-1)}^n \}_{n=1}^\infty$ is the sequence
...
\begin{example}
The sequence $\{ {(-1)}^n \}_{n=1}^\infty$ is divergent.  Proof: If there
...
Suppose the sequence $\{ x_n \}_{n=1}^\infty$ has limits $x$ and $y$.
...
The sequence $\{ {(-1)}^n \}_{n=1}^\infty$ shows that the converse
...
decreasing, and $\{ {(-1)}^n \}_{n=1}^\infty$ is not monotone.
...
define $y_n := x_{n+K}$.  We wish to show that $\{ y_n \}_{n=1}^\infty$ converges
to $x$.
...
$y_n := x_{n+K}$, and suppose that $\{ y_n \}_{n=1}^\infty$ converges to $x \in \R$.
...
\begin{proof}
Let $x := \lim_{n\to\infty} a_n = \lim_{n\to\infty} b_n$.
...
\begin{proof}
Let $x := \lim_{n\to\infty} x_n$ and $y := \lim_{n\to\infty} y_n$. 
Let 
...
The sequence $\{ z_n \}_{n=1}^\infty$, where $z_n := x_n + y_n$, converges and
\begin{equation*}
...
the sequence $\{ z_n \}_{n=1}^\infty$, where $z_n := \dfrac{x_n}{y_n}$, converges and
\begin{equation*}
...
write $z_n := x_n + y_n$.  Let $x := \lim_{n\to\infty} x_n$,
$y := \lim_{n\to\infty} y_n$, and $z := x+y$.
...
\begin{proof}
Let $\{ x_n \}_{n=1}^\infty$ be a convergent sequence and let $x :=
\lim_{n\to\infty} x_n$.
...
\begin{example}
Let $\{ x_n \}_{n=1}^\infty$ be defined by $x_1 := 2$ and
\begin{equation*}
...
same limit.  Let us define $x := \lim_{n\to\infty} x_n$.  Take the limit of
...
Let $x := \lim_{n\to\infty} c^n$.  The 1-tail $\{ c^{n+1} \}_{n=1}^\infty$
...
$L=1$.  The sequence $\{ {(-1)}^n \}_{n=1}^\infty$ does not converge at all, and $L=1$ as
well.
...
the $M$-tail of 
$\{x_n\}_{n=1}^\infty$ converges to zero and therefore $\{x_n\}_{n=1}^\infty$ converges to zero.
...
$w_n := \frac{y_n}{x_n}$.  Do $\{ z_n \}_{n=1}^\infty$ and $\{ w_n \}_{n=1}^\infty$
...
the sequence $\{ {(-1)}^n \}_{n=1}^\infty$ is bounded,
...
subsequences of $\{ x_n \}$, nor do they have to even 
...
Let $n_1 := 1$, and suppose $n_1,n_2,\ldots,n_{k-1}$ are already defined for
...
Set $n_{k} :=  m$.  The subsequence $\{ x_{n_k} \}$ is defined.  Next, we
...
\end{equation*}
Take $M := \max \{M_1 , M_2 , 2 \}$.  For all $k \geq M$,
\begin{equation*}
\begin{split}
\abs{x- x_{n_k}} & =
...
\end{equation*}
First suppose
$\liminf \, x_n = \limsup \, x_n$.  Then $\{ a_n \}$ and $\{ b_n \}$
...
as usual.
Also define
$c_n := \sup \{ x_{n_k} : k \geq n \}$.
It is not true that $\{ c_n \}$ is necessarily a subsequence of $\{ a_n \}$.  However,
...
Suppose $\{x_n\}$ is decreasing and unbounded.
...
\begin{samepage}
\begin{exercise}
Let $\{ x_n \}$ and $\{ y_n \}$ be bounded sequences (by the previous
exercise, $\{ x_n + y_n \}$ is bounded).
\begin{enumerate}[a)]
\item
Show that
\begin{equation*}
...
The series $\sum \nicefrac{1}{n}$ diverges (despite the fact that $\lim
\nicefrac{1}{n} = 0$).  This is the famous \emph{\myindex{harmonic series}}%
...
we get
$\displaystyle \lim_{n\to\infty} \abs{x_{n+k}-x_n} = 0$, yet $\{ x_n
\}_{n=1}^\infty$ is not Cauchy.
\end{exercise}
...
Alternatively, note that if $L=\infty$, then $\bigl\{ {\abs{x_{n}}}^{1/n}
\bigr\}_{n=1}^\infty$
...
\begin{proof}
Let $s_m := \sum_{k=1}^m {(-1)}^n x_n$ be the $m$th partial sum.  Then
\begin{equation*}
s_{2k} =
\sum_{n=1}^{2k} {(-1)}^n x_n
=
...
\end{equation*}
The sequence $\{ x_n \}$ is decreasing and so $(-x_{2\ell-1}+x_{2\ell}) \leq 0$
for all $\ell$.
Therefore, the subsequence $\{ s_{2k} \}$ of partial sums
is a decreasing sequence.  Similarly, $(x_{2\ell}-x_{2\ell+1}) \geq 0$, and so
\begin{equation*}
s_{2k} = - x_1 + ( x_2 - x_3 ) + \cdots + ( x_{2k-2} - x_{2k-1} ) + x_{2k}
\geq -x_1 .
\end{equation*}
The sequence $\{ s_{2k} \}$ is decreasing and bounded below, so it converges.
Let $a := \lim\, s_{2k}$.
...
$R := \lim_{n\to\infty} \frac{\abs{a_{n+1}}}{\abs{a_n}}$ exists or is $\infty$.
...
otherwise
let $a_n := 0$.  Show that $\sum_{n=1}^\infty a_n$ diverges in the usual sense
