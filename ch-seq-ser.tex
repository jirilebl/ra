...
\leavevmode
\begin{enumerate}[(i)]
\item If $\{ x_n \}$ is a convergent sequence such that $x_n \geq 0$ for all
$n \in \N$,
then
\begin{equation*}
...
Let $x := \lim\, c^n$.  The 1-tail $\{ c^{n+1} \}$
...
\begin{exercise}
Suppose $\{ x_n \}$ is a sequence, $x \in \R$, and $x_n \not= x$ for
...
\begin{enumerate}[a)]
\item
Suppose $\{ a_n \}$ is bounded and $\{ b_n \}$ converges to 0.
Show that $\{ a_n b_n \}$ converges to 0.
\item
...
\noindent
\emph{\textbf{Theorem.} Suppose $\sum x_n$ is a series whose sequence of partial sums
is bounded, $\{ \lambda_n \}$ is a sequence with $\lim \lambda_n = 0$, and
